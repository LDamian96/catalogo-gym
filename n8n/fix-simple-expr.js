...
// Code node extracts phone, HTTP node sends
const extractCode = [
  ...
  "return [{json: {phone: phone, text: text, name: data.pushName || 'Cliente'}}];"
].join("\n");
    ...
    nodes: [
      ...
      },
      {
        ...
        parameters: {
          ...
          contentType: "json",
          specifyBody: "json",
          jsonBody: '={"number":"{{ $json.phone }}","textMessage":{"text":"Hola {{ $json.name }}! Soy ProVitamins Peru. Proteinas desde S/89, creatina desde S/49. Visita: https://calogojh.duckdns.org"}}'
        }
      }
    ],
    connections: {
