...
login(cookie => {
  // Super simple: Webhook -> HTTP Request con TODO hardcoded (sin expresiones)
  patch(cookie, "/rest/workflows/botultra", {
    ...
    nodes: [
      ...
      },
      {
        ...
        parameters: {
          ...
          sendBody: true,
          specifyBody: "string",
          body: '{"number":"51929751690","textMessage":{"text":"BOT FUNCIONA! Este es un mensaje hardcoded de prueba."}}'
        }
      }
    ],
    connections: {
    ...
    // Activate
    setTimeout(() => {
      patch(cookie, "/rest/workflows/botultra", {active: true}, (s2, b2) => {
        var r = JSON.parse(b2);
        console.log("Active:", r.data && r.data.active);
      });
    }, 1000);
  });
});
